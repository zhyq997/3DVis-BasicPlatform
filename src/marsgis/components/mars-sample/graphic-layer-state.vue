<template>
  <div class="f-mb">
    <div>
      <a-space>
        ...
        <a-checkbox
          v-model:checked="formState.enabledShowHide"
          @change="onChangeShow"
          title="显示隐藏状态"
          >显示</a-checkbox
        >
    ...
    <div>
      <a-space>
        <span
          v-if="formState.enabledOpacity"
          class="mars-pannel-item-label"
          title="不是所有矢量数据均支持修改全局透明度"
          >透明度:</span
        >
        <mars-slider
          v-if="formState.enabledOpacity"
          v-model:value="formState.opacity"
          :min="0.0"
          :max="1.0"
          :step="0.1"
  ...
  <div class="f-mb" v-if="props.interaction">
    <a-space>
      ...
      <a-checkbox
        v-model:checked="formState.enabledPopup"
        @change="onChangePopup"
        title="是否绑定Popup鼠标单击弹窗"
        >单击Popup</a-checkbox
      >
      <a-checkbox
        v-model:checked="formState.enabledTooltip"
        @change="onChangeTooltip"
        title="是否绑定Tooltip鼠标移入弹窗"
        >移入Tooltip</a-checkbox
      >
    </a-space>

    <a-checkbox
      class="rightMenu-checkbox f-pt"
      v-model:checked="formState.enabledRightMenu"
      @change="onChangeRightMenu"
      title="是否绑定右键菜单"
      >右键菜单</a-checkbox
    >
  </div>
  ...
  <div class="f-mb" v-if="props.enabledDraw">
    <a-space>
      ...
      <mars-button
        :class="props.drawLabel2 && !formState.isDrawing ? 'data-maintain-two' : 'data-maintain'"
        v-if="!formState.isDrawing"
        @click="onClickStartDraw"
        >{{ props.drawLabel1 }}</mars-button
      >
      <mars-button
        :class="props.drawLabel2 && !formState.isDrawing ? 'data-maintain-two' : 'data-maintain'"
        v-if="props.drawLabel2 && !formState.isDrawing"
        @click="onClickStartDraw2"
        >{{ props.drawLabel2 }}</mars-button
      >
      <mars-button class="data-maintain" v-if="formState.isDrawing" @click="onClickClearDrawing"
        >取消绘制</mars-button
      >
    </a-space>
  </div>
  ...
  <div class="data-edit">
    ...
    <a-checkbox
      v-if="enabledTable"
      v-model:checked="formState.hasTable"
      title="显示图层内所有矢量数据列表"
      >显示列表</a-checkbox
    >
  </div>
  ...
  <div class="f-mb f-pt">
    <a-space>
      ...
      <mars-input-number :min="1" :max="1000000" v-model:value="formState.count" step="1" />条
  ...
  <div class="f-mb">
    <a-space>
      ...
      <mars-button @click="expGeoJSONFile" title="保存GeoJSON"> 导出GeoJSON </mars-button>
    ...
    >
      <mars-button class="open-file-btn" title="打开GeoJSON"> 打开 </mars-button>
    </a-upload>
  </div>
  ...
  <div class="f-mb data-list">
    <mars-table
      ...
      :pagination="{ pageSize: currentPage, simple: true }"
