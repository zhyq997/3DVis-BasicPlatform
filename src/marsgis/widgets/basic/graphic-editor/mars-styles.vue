<template>
  <a-collapse :key="globalKey" v-model:activeKey="styleCollapse" expandIconPosition="end">
        ...
        <template v-if="styleValue">
          <template v-for="(item, i) in viewStyles" :key="i">
            ...
            <template v-if="item.type !== 'hidden'">
              ...
              <!-- 材质相关属性 -->
              <template
                v-if="item.name === 'materialType' && viewMaterials && styleValue.materialOptions"
              >
    ...
    <a-collapse-panel
      v-if="styleValue && styleValue.label"
      key="2"
      :showArrow="false"
      header="+ 注记信息"
    >
...
<script lang="ts" setup>
  ...
  function setDefault() {
    ...
    originStyles.forEach((item: any) => {
      if (item.next) {
        ...
        styleValue.value[item.name][item.next] =
          styleValue.value[item.name][item.next] ?? val ?? getViewDefval(item, styleValue.value); // 数据中没有的地方使用默认值
    ...
    if (item.next && item.next === 'materialType') {
      ...
      if (selectOptions) {
        const mOp = selectOptions.find(
          (item) => item.value === styleValue.value[name].materialType,
        ); // 当前选项的材质属性配置
        if (mOp) {
          defval = mOp.defval ?? {};
        }
  ...
  function updateNextMaterials(parent) {
    ...
    nextViewMaterials.value.forEach((item) => {
      materialOptions[item.name] =
        styleValue.value[parent].materialOptions[item.name] ??
        getViewDefval(item, styleValue.value[parent].materialOptions);
    });
    ...
    if (originLabels) {
      originLabels.forEach((item: any) => {
        styleValue.value.label[item.name] =
          styleValue.value.label[item.name] ?? getViewDefval(item, styleValue.value.label); // 数据中没有的地方使用默认值
      });
    }
  }
  // 更新属性的显示隐藏，通过配置中的show属性来控制
