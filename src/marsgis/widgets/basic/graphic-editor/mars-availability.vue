      ...
      availability.value = Array.isArray(data)
        ? data?.map((julianObj) => {
            ...
            for (const key in julianObj) {
              ...
              if (julianObj[key]) {
                value = isJulianDate
                  ? dayjs(mapWork.julianToDate(julianObj[key]))
                  : dayjs(julianObj[key]);
              }
  ...
  const disabledDate = (current: Dayjs, index: number, key: string, isCompare = false) => {
    ...
    if (isCompare) {
      if (startTime && current < startTime) {
        return {
          isChange: false,
          message: `请重新选择时间，须大于等于${dayjs(startTime).format('YYYY-MM-DD HH:mm:ss')}`,
        };
      } else if (endTime && current > endTime) {
        return {
          isChange: false,
          message: `请重新选择时间，须小于等于${dayjs(endTime).format('YYYY-MM-DD HH:mm:ss')}`,
        };
      } else {
    ...
    const time =
      availability.value?.map((dayjsDate) => {
        ...
        for (const key in dayjsDate) {
          timeObj[key] = dayjsDate[key]
            ? dayjs(dayjsDate[key]).format('YYYY-MM-DD HH:mm:ss')
            : null;
        }
