<template>
  <mars-dialog :visible="true" right="10" top="60" width="380" title="模型压平">
    <div>
      <a-row :gutter="[5, 10]">
        <a-col :span="6">
          ...
          <mars-table :pagination="{ pageSize: 5 }" :row-selection="rowSelection" :dataSource="dataSource"
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'caozuo'">
                <a-space>
                  <mars-icon icon="move-one" class="icon-vertical-a" @click="flyto(record)" />
                  <mars-icon icon="delete" class="icon-vertical-a" @click="deleted(record)" />
                </a-space>
              </template>
              <template v-else>
                {{ record.name }}
